...
const shouldContinue = (input: unknown) => {
  const state = input as { agentState: AgentState };
  const result = state.agentState.critique === undefined ? "__end__" : "__start__";
  return result;
};

const workflow = new StateGraph({
  channels: { __root__: agentState },
});
...
workflow.addEdge("__start__", "__start__");
